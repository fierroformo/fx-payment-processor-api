...
class ValidateFund:
    ...
    def validate(currency: str, amount: float) -> Optional[Tuple]:
        if not currency in AVAILABLE_CURRENCIES:
            return Messages.UNKNOWN_CURRENCY, HTTPStatus.HTTP_400_BAD_REQUEST
        elif isinstance(amount, str):
            return Messages.INVALID_AMOUNT, HTTPStatus.HTTP_400_BAD_REQUEST
        elif amount < 0:
            return Messages.NEGATIVE_AMOUNT, HTTPStatus.HTTP_400_BAD_REQUEST
...
class ValidateConvert:
    ...
    def validate(
        ...
        if not to_currency in AVAILABLE_CURRENCIES or not from_currency in AVAILABLE_CURRENCIES:
            ...
            return Messages.INSUFFICIENT_FUNDS, HTTPStatus.HTTP_400_BAD_REQUEST
